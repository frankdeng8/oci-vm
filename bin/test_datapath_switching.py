...
qmp_path = os.path.join(path, "..", "qmp")
...
class DatapathSwitch (threading.Thread):
    ...
    def run(self):
        ...
        # FAILOVER and DEVICE_DELETED and RESUME event
        if event.find('FAILOVER_') == 0 or event == 'DEVICE_DELETED' or event == 'RESUME':
            ...
            if event == 'RESUME':
                ...
                output = run_cmd(["./vf.sh", "-P",  vm_dir, "-o", "up"])
                _print(output)
                # attach vf
                _print("1-(c) hot add a VF with 'x-failover-primary' property set to true")
                output = run_cmd(["./vf.sh", "-P",  vm_dir, "-o", "add"])
                _print(output)
            ...
            if event == 'FAILOVER_PRIMARY_CHANGED' and device == 'vf0' and enabled is True:
                ...
                _print("1-(d) once VF shows up in the guest, a corresponding FAILOVER_PRIMARY_CHANGED VF 'enabled' is emitteis emittedd.")
                _print("Hippovisor should remove the conflict MAC filter for VF to activate its datapath later: ip link set macvtap down")
                output = run_cmd(["./macvtap.sh", "-P", vm_dir, "-o", "down"])
                _print(output)
            ...
            if event == 'FAILOVER_PRIMARY_CHANGED' and device == 'vf0' and enabled is False:
                ...
                output = run_cmd(["./macvtap.sh", "-P", vm_dir, "-o", "up"])
                _print(output)
            ...
            if event == 'DEVICE_DELETED' and device == 'vf0':
                ...
                output = run_cmd(["./vf.sh", "-P",  vm_dir, "-o", "down"])
                _print(output)
            ...
            if event == 'FAILOVER_UNPLUG_PRIMARY' and device == 'net0':
                ...
                _print("Event: %s, device: %s" % (event, device))
                output = run_cmd(["./vf.sh", "-P",  vm_dir, "-o", "del"])
                _print(output)
