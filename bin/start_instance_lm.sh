...
start_instance () {
  ...
  -device virtio-net-pci,netdev=hostnet0,id=net0,mac=$mac,standby=on,romfile=$dir/qemu-img-binaries/1af41000.efirom
  -netdev tap,fd=$tap_fd,id=hostnet0,vhost=on,vhostfd=20"

  #-device vfio-pci,host=$vf_pci_addr,id=vf0,romfile=$dir/qemu-img-binaries/$vf_rom

  #-device pci-bridge,chassis_nr=1,id=pci.1,bus=pci.0,addr=0x11
  #-device pci-bridge,chassis_nr=2,id=pci.2,bus=pci.0,addr=0x12
  #-device vfio-pci,host=$vf_pci_addr,id=vf0,bus=pci.1,addr=0x1,romfile=$dir/qemu-img-binaries/$vf_rom
  #-device virtio-net-pci,netdev=hostnet0,id=net0,bus=pci.2,addr=0x1,mac=$mac,standby=on,romfile=$dir/qemu-img-binaries/1af41000.efirom
  #-netdev tap,fd=$tap_fd,id=hostnet0,vhost=on,vhostfd=20"

    echo "Starting VM with [cores:$cpu, memory: $memory MB,  VNC: $HOSTNAME:$vnc_port]"
    echo "qemu-system-x86_64 cmdline:"
    echo "  /usr/bin/qemu-system-x86_64 $args 3<>/dev/tap$tap_id 20<>/dev/vhost-net"
    ...
    echo
    echo "To attach VF in QMP: device_add vfio-pci,host=$vf_pci_addr,id=vf0,romfile=$dir/qemu-img-binaries/$vf_rom"

    /usr/bin/qemu-system-x86_64 $args 3<>/dev/tap$tap_id 20<>/dev/vhost-net
...
echo "Creating macvtap macvtap$vf_id on $pf_dev"
#ip link add link $pf_dev name macvtap$vf_id address $mac type macvtap
#ip link add link $pf_dev name macvtap$vf_id address $mac type macvtap mode vepa
ip link add link $pf_dev name macvtap$vf_id address $mac type macvtap mode bridge
ip link set macvtap$vf_id up
ip link show macvtap$vf_id

#tap_id=$(ip link show macvtap$vf_id | head -1 | awk -F: '{print $1}')
tap_id=$(cat /sys/class/net/macvtap$vf_id/ifindex)
vhost_fd=20
tap_fd=3
